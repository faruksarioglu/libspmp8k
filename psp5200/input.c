...
void INPUT_Frame(void)
{
	...
	scanline_counter = 10000;	/* 10000 idi do nothing in INPUT_Scanline() */
	...
	*/
    ...
    if (c.keys & GE_KEY_DOWN) key_code=AKEY_DOWN;
	if (c.keys & GE_KEY_UP) key_code=AKEY_UP;
	if (c.keys & GE_KEY_LEFT) key_code=AKEY_LEFT;
	if (c.keys & GE_KEY_RIGHT) key_code=AKEY_RIGHT;
	if (c.keys & GE_KEY_X){ fire=1; key_code=AKEY_F1;}
	if (c.keys & GE_KEY_O) key_code=AKEY_5200_0;
	if (c.keys & GE_KEY_START) key_code=AKEY_5200_START;
	if (c.keys & GE_KEY_SELECT) key_code=AKEY_F2;
...
//	for (i = 0; i < 1; i++) {
		...
		TRIG_input[i] = fire||Atari_TRIG(i);
...
/*	switch (mouse_mode) {
	...
	*/
	...
	if (joy_multijoy && machine_type != MACHINE_5200) {
		...
		PORT_input[1] = 0xff;
		TRIG[0] = TRIG_input[joy_multijoy_no]||fire;
		TRIG[2] = TRIG[1] = 1;
		TRIG[3] = machine_type == MACHINE_XLXE ? cartA0BF_enabled : 1;
	}
	else {
		TRIG[0] = TRIG_input[0]||fire;
		TRIG[1] = TRIG_input[1];
