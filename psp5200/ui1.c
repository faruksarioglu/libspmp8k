...
const char* select_file(const char *start, const char *extension, int font_size)
{
   char *filename;
    ...
    dents = malloc(sizeof(struct _ecos_dirent) * 60);
    stats = malloc(sizeof(struct _ecos_stat) * 60);
    int num_dents = 60;
...
reload_dir:
    ...
    int current_entry = 0;
    	...
    	uint16_t *fba = psp_sdl_get_vram();//gDisplayDev->getShadowBuffer();
	...
	psp_sdl_flip();
        for (i = current_top;
            ...
            if (_ECOS_S_ISDIR(stats[i].st_mode)) {
                psp_sdl_put_char(20, (i - current_top) * 15, 13, 0, '[', 1, 0);
			    psp_sdl_print(25,(i - current_top) * 15,dents[i].d_name,13);
                psp_sdl_put_char(150, (i - current_top) * 15, 13, 0, ']', 1, 0);
			   // psp_sdl_flip();
			//	cache_sync();

            }
            else {
               // psp_sdl_fill_rectangle(0,(i - current_top) * 15,480,15,0,0);
				//psp_sdl_print(25,(i - current_top) * 15,"                                   ",23 );
			    psp_sdl_print(25,(i - current_top) * 15,dents[i].d_name,23);
			//	cache_sync();
            }
        }
        psp_sdl_flip();
        invert(fba + (current_entry - current_top) * 480 * 15, 15 * 480);
       // cache_sync();
       psp_sdl_flip();
      ...
      // psp_sdl_flip();
	    ...
	    if ((keys.keys & GE_KEY_UP) && current_entry > 0) {
            ...
            current_entry++;
            if ((current_entry - current_top + 1) * 15 > 272)
                current_top++;           
			//	cache_sync();
        }
...
int select_cart_type(int r)
{
    int dent_count=16;
	 int f;
 struct atari_key_cart_types {
   char name[30];
   int type;
 };

  struct atari_key_cart_types cart_types[16]=
  {
    { "Standard 8 KB     ", 1 },
    { "Standard 16 KB    ", 2 },
    { "Standard 32 KB 5200", 4 },
    { "Two chip 16 KB 5200", 6 },
     { "One chip 16 KB 5200", 16 },
    { "Bounty Bob 40 KB 5200 ", 7 },
    { "Express 64 KB cartridge", 9 },
    { "XEGS 32 KB cartridge", 12 },
    { "XEGS 64 KB cartridge", 13 },
    { "XEGS 128 KB cartridge", 14 },
     { "Standard 8 KB 5200", 19 },
      { "Standard 4 KB 5200", 20 },
     { "XEGS 256 KB cartridge", 23 },
     { "MegaCart 16 KB cartridge", 26 },
     { "MegaCart 32 KB cartridge", 27 },
	 { "Phoenix 8 KB cartridge",39	}};
	
    int max_entries_displayed = 480 / 15;
    int current_top = 0;
    int current_entry = 0;

      	uint16_t *fb = psp_sdl_get_vram();//gDisplayDev->getShadowBuffer();
        psp_sdl_black_screen();
	for (;;) {
        int i;
        psp_sdl_black_screen();
		...
		for (i = current_top;
             i < current_top + max_entries_displayed && i < current_top + dent_count; i++) {
                psp_sdl_fill_rectangle(0,(i - current_top) * 15,480,15,0,0);
				psp_sdl_print(10,(i - current_top) * 15,"                                   ",23 );

				psp_sdl_print(10,(i - current_top) * 15,cart_types[i].name,23 );
        
	    }
    psp_sdl_flip();
        invert(fb + (current_entry - current_top) * 480 * 15, 15 * 272);
    psp_sdl_flip();
    ...
    psp_sdl_flip();
        ...
        if ((keys.keys & GE_KEY_DOWN) && current_entry < dent_count - 1) {
            current_entry++;
            if ((current_entry - current_top + 1) * 15 > 272)
                current_top++;
        }
        if (keys.keys & GE_KEY_X) {
            f = cart_types[current_entry].type;
			  	break;
        }
    }
    psp_sdl_black_screen();
