...
static  Uint8 dsp_buffer[FRAGSIZE]; /* x2, because 16bit buffers */

int psp_buffer_length,psp_buffer_read_index  = 0,
...
int KBD_TRIG_0 = GE_KEY_X;
int KBD_STICK_0_LEFT = GE_KEY_LEFT;
int KBD_STICK_0_RIGHT = GE_KEY_RIGHT;
int KBD_STICK_0_DOWN = GE_KEY_DOWN;
int KBD_STICK_0_UP = GE_KEY_UP;
...
int KBD_TRIG_1 = GE_KEY_X;
int KBD_STICK_1_LEFT = GE_KEY_LEFT;
int KBD_STICK_1_RIGHT = GE_KEY_RIGHT;
int KBD_STICK_1_DOWN = GE_KEY_DOWN;
int KBD_STICK_1_UP = GE_KEY_UP;
...
#define minjoy 100      /* real joystick tolerancy */
...
{
	Pokey_process_8(dsp_buffer, 4096);
 emuIfSoundPlay(&sp);
...
{
  Pokey_process_8(dsp_buffer, 2048);
//Pokey_process_8
  if (ATARI.atari_snd_enable) {
...
SDL_Sound_Initialise(void)
{
 ...
 psp_buffer_length = 4096;
  sp.rate =44100;//31400;
  sp.callback = 0;//psp_sound_callback;
  sp.buf_size = psp_buffer_length;
  sp.channels = 2;		/* mono */
  sp.depth = 16;		/* SQ_Max/3 */
  sp.buf=(uint8_t*)dsp_buffer;
  emuIfSoundInit(&sp);
  //dsp_buffer = (uint16_t *)malloc(psp_buffer_length * sizeof(uint16_t)); 
  //memset(dsp_buffer, 0, psp_buffer_length);
 ...
 */
 sound_flags |= SND_BIT16;
  ...
  Pokey_sound_init(FREQ_17_EXACT, 44100, 2, sound_flags);
...
Display_normal(void)
{
  ...
  u32    *psp_scr = 80+psp_top_scr+(20*480);
...
atari_synchronize()
{
  ...
  u32 curclock = libgame_utime;//SDL_GetTicks();

  if (ATARI.atari_speed_limiter) {
    while (curclock < nextclock) {
     curclock = libgame_utime();
    }
...
Atari_DisplayScreen(void)
{
  if (ATARI.psp_skip_cur_frame <= 0) {
    ...
    if (ATARI.atari_view_fps) {
      char buffer[32];
      sprintf(buffer, "%03d %3d fps", ATARI.atari_current_clock, (int)ATARI.atari_current_fps );
...
ge_key_data_t c;
    NativeGE_getKeyInput4Ntv(&c);
    if (c.keys & GE_KEY_LEFT) {
	stick0 = STICK_LEFT;stick1 = 0x80;}
	
	if (c.keys & GE_KEY_RIGHT) {stick0 = STICK_RIGHT;//stick1 = 0x40;
	}
     if (c.keys & GE_KEY_UP) {stick0 = STICK_FORWARD ;//stick1 = 0x10;
	}
     if (c.keys & GE_KEY_DOWN) {stick0 = STICK_BACK;//stick1 = 0x20;
	}
    
	if (c.keys & GE_KEY_LEFT&GE_KEY_UP) {
	stick0 = STICK_LEFT;stick1 = STICK_UL;}
if (c.keys & GE_KEY_RIGHT&GE_KEY_UP) {
	stick0 = STICK_LEFT;stick1 = STICK_UR;}
     if (c.keys & GE_KEY_DOWN& GE_KEY_RIGHT) {stick0 = STICK_LR;//stick1 = 0x20;
	}
    if (c.keys & GE_KEY_DOWN& GE_KEY_LEFT) {stick0 = STICK_LL;//stick1 = 0x20;
	}
    //else 
...
ge_key_data_t c;
    NativeGE_getKeyInput4Ntv(&c);
     if (c.keys & GE_KEY_X) {trig0=1;trig1=1;//stick1 = 0x20;
	}
...
void CountFPS(void)
{
  ...
  if (ticks1 == 0)
    ticks1 = libgame_utime();
  ticks2 = libgame_utime();
  shortframes++;
