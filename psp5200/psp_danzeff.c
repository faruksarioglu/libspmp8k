...
*/
unsigned int
danzeff_readInput(ge_key_data_t* pspctrl)
{
  ...
  int x = 1;
  int y = 1;
  if (pspctrl->keys & GE_KEY_LEFT ) x -= 1;
  if (pspctrl->keys & GE_KEY_RIGHT) x += 1;
  if (pspctrl->keys & GE_KEY_UP   ) y -= 1;
  if (pspctrl->keys & GE_KEY_DOWN)  y += 1;
  ...
  if (!holding)
  {
    if (pspctrl->keys& (GE_KEY_X|GE_KEY_O|GE_KEY_TRIANGLE|GE_KEY_SQUARE)) //pressing a char select button
    {
      int innerChoice = 0;
      if      (pspctrl->keys & GE_KEY_TRIANGLE)
        innerChoice = 0;
      else if (pspctrl->keys & GE_KEY_SQUARE)
        innerChoice = 1;
      else if (pspctrl->keys & GE_KEY_X)
        innerChoice = 2;
      else //if (pspctrl.keys & GE_KEY_O)
        innerChoice = 3;
      
      //Now grab the value out of the array
      pressed = modeChar[ (mode*2 + shifted) % MAX_VKEYBOARD][y][x][innerChoice];
    }
    else if (pspctrl->keys& GE_KEY_L1) //toggle mode
    {
      dirty = true;
      mode++;
      mode %= MODE_COUNT;
    }
    else if (pspctrl->keys& GE_KEY_SELECT)
    {
      pressed = DANZEFF_SELECT; //SELECT
    }
    else if (pspctrl->keys& GE_KEY_START)
    {
...
//RTRIGGER doesn't set holding
  holding = pspctrl->keys & ~(GE_KEY_UP|GE_KEY_DOWN|GE_KEY_LEFT|GE_KEY_RIGHT); 
...
danzeff_init_skin()
{
  psp_kbd_last_skin = 1;//psp_fmgr_get_dir_list(PICS_BASEDIR, GP2X_KBD_MAX_SKIN, psp_kbd_skin_dir) - 1;
