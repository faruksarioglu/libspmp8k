...
psp_sdl_get_vram()
{
  ...
  return (ushort *)back_surface->pixels;;
}
...
{
  if (! splash_surface) {
    splash_surface = IMG_Load("/fat20a2/hda2/GAME/5200/splash.png");
  }
...
{
  if (! help_surface) {
    help_surface = IMG_Load("/fat20a2/hda2/GAME/5200/help.png");
  }
...
psp_sdl_display_splash()
{
  ...
  psp_sdl_blit_splash();
  psp_sdl_print(x, y, "MERHABA", col);
  psp_sdl_flip();

  psp_sdl_blit_splash();
  psp_sdl_print(x, y, "2.2", col);
  psp_sdl_flip();


  while (index < 50) {
    NativeGE_getKeyInput4Ntv(&c);
    if (c.keys & (GE_KEY_START|GE_KEY_X)) break;
    index++;
  }
}
