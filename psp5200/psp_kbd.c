...
#include <libgame.h>
# define KBD_MIN_ANALOG_TIME  150
# define KBD_MIN_START_TIME   80
# define KBD_MAX_EVENT_TIME   50
# define KBD_MIN_PENDING_TIME 300
# define KBD_MIN_HOTKEY_TIME  1000
# define KBD_MIN_DANZEFF_TIME 150
# define KBD_MIN_COMMAND_TIME 100
# define KBD_MIN_BATTCHECK_TIME 90000
# define KBD_MIN_AUTOFIRE_TIME   10

 static ge_key_data_t    loc_button_data;
 ...
 {
   GE_KEY_UP         , /*  KBD_UP         */
   GE_KEY_RIGHT      , /*  KBD_RIGHT      */
   GE_KEY_DOWN       , /*  KBD_DOWN       */
   GE_KEY_LEFT       , /*  KBD_LEFT       */
   GE_KEY_TRIANGLE   , /*  KBD_TRIANGLE   */
   GE_KEY_O     , /*  KBD_CIRCLE     */
   GE_KEY_X      , /*  KBD_CROSS      */
   GE_KEY_SQUARE     , /*  KBD_SQUARE     */
   GE_KEY_SELECT     , /*  KBD_SELECT     */
   GE_KEY_START      , /*  KBD_START      */
   GE_KEY_L1   , /*  KBD_LTRIGGER   */
   -1   , /*  KBD_RTRIGGER   */
 };
...
atari_key_event(int atari_idx, int button_press)
{
  ...
  if ((atari_idx >= ATARIC_FPS) &&
    ...
    if (button_press) {
      ge_key_data_t c;
      //NativeGE_getKeyInput4Ntv
      //NativeGE_getKeyInput4Ntv(&c);
      if ((libgame_utime - loc_last_hotkey_time) > KBD_MIN_HOTKEY_TIME) 
      {
        loc_last_hotkey_time = libgame_utime;
...
psp_kbd_enter_command()
{
  ...
  if (command_atari_pending) 
  {
   ...
   if ((libgame_utime - loc_last_event_time) > KBD_MIN_COMMAND_TIME) {
      loc_last_event_time = libgame_utime;
      command_atari_pending = 0;
...
psp_kbd_enter_danzeff()
{
  ...
  if (danzeff_atari_pending) 
  {
   ...
   if ((libgame_utime - loc_last_event_time) > KBD_MIN_PENDING_TIME) {
      loc_last_event_time = libgame_utime;
  ...
  if ((libgame_utime - loc_last_event_time) > KBD_MIN_DANZEFF_TIME) {
    loc_last_event_time = libgame_utime;
...
kbd_scan_keyboard(void)
{
  ...
  if (ATARI.atari_auto_fire) {
    delta_stamp = libgame_utime() - first_time_auto_stamp;
    if ((delta_stamp < 0) || 
        (delta_stamp > (KBD_MIN_AUTOFIRE_TIME / (1 + ATARI.atari_auto_fire_period)))) {
      first_time_auto_stamp = libgame_utime();
  ...
  {
    if (c.keys & loc_button_mask[b]) {
# if 0  //GAME MODE !
      if (!(loc_button_data.keys & loc_button_mask[b])) 
# endif
      {
  ...
  memcpy(&loc_button_data,&c,sizeof(ge_key_data_t));
...
psp_update_keys(void)
{
  ...
  if (first_time) {
    ...
    if (first_time_stamp == -1) first_time_stamp = libgame_utime;
    ...
    NativeGE_getKeyInput4Ntv(&loc_button_data);
...
/*NativeGE_getKeyInput4Ntv(&keys);
        if (keys.keys & GE_KEY_DOWN) {
        	loc_button_press[ATARIK_DOWN] = 1;//=ATARIK_JOY_DOWN;
       		}
    
	 if (keys.keys & GE_KEY_UP) {
        	loc_button_press[ATARIK_UP] = 1;;
       		}
     if (keys.keys & GE_KEY_LEFT) {
        	loc_button_press[ATARIK_LEFT] = 1;
       		}
     if (keys.keys & GE_KEY_RIGHT) {
	loc_button_press[ATARIK_RIGHT] = 1;       		}
     if (keys.keys & GE_KEY_X) {
    loc_button_press[ATARIK_RETURN] = 1;;
       		}
     if (keys.keys & GE_KEY_START) {
        loc_button_press[ATARIK_JOY_FIRE1] = 1;;
       		}
       		if (keys.keys & GE_KEY_O) {
        loc_button_press[AKEY_EXIT] = 1;;
       		}
