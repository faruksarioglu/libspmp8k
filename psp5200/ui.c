...
//LUDO:
static int new_card_loaded = 0;
static char new_card_filename[MAX_PATH];
...
static void CartManagement(void)
{
	...
	for (;;) {
        ...
        new_card_loaded = 1;
        strcpy(new_card_filename, cart_filename);
				Coldstart();
				return;
			}
			break;
...
int psp_skip_max_frame = ATARI.psp_skip_max_frame;
...
ATARI.psp_skip_cur_frame = 0;
 ...
 NativeGE_getKeyInput4Ntv(&c);
    if (c.keys & GE_KEY_UP) alt_function++ ;
if (c.keys & GE_KEY_DOWN) alt_function-- ;
if (c.keys & GE_KEY_LEFT) option=12;//back
if (c.keys & GE_KEY_O) option=MENU_EXIT;//back
...
//LUDO:
  ...
  ATARI.psp_skip_max_frame = psp_skip_max_frame;
  ATARI.psp_skip_cur_frame = 0;
  if (new_card_loaded) {
    atari_load_rom_settings(new_card_filename);
  }
  new_card_loaded = 0;
}
