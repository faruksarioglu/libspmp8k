...
//	if (options.vector_width == 0) 
	options.vector_width = ODX_SCREEN_WIDTH;
	//if (options.vector_height == 0) 
	options.vector_height = ODX_SCREEN_HEIGHT;
	//if (options.vector_width == 0) options.vector_width = 800;
	//if (options.vector_height == 0) options.vector_height = 600;
//	options.norotate=1;
...
static int vh_open(void)
{
	...
	if (Machine->orientation & ORIENTATION_SWAP_XY)
	{
		...
		printf("vh open creating display");// cyg_thread_delay(200);
...
int run_machine(void)
{
	...
	printf("vh_open ok \n"); //cyg_thread_delay(200);
		if(tilemap_init()==0)
	printf("tilemap init ok \n"); else printf("tilemap init problem \n") ;// cyg_thread_delay(200);
		sprite_init();
	printf("Sprite init ok \n"); //cyg_thread_delay(200);
		gfxobj_init();
	printf("gfx obj init ok \n");// cyg_thread_delay(200);
		if (drv->vh_start == 0 || (*drv->vh_start)() == 0)      /* start the video hardware */
		{
			printf("Vh start ok \n");// cyg_thread_delay(200);
			if (sound_start() == 0) /* start the audio hardware */
			{
				printf("Sound start ok \n");//cyg_thread_delay(200);
			//	odx_text_log("starting game...");cyg_thread_delay(200);
				int	region;
				...
				//	osd_led_w(2,0);
				//	osd_led_w(3,0);
//printf("before init user interface  \n");//cyg_thread_delay(200);

					init_user_interface();
//printf("init user interface ok \n");//cyg_thread_delay(200);
					/* disable cheat if no roms */
				//	if (!gamedrv->rom) options.cheat = 0;

				//	if (options.cheat) InitCheat();
//printf("init cheat ok \n");//cyg_thread_delay(200);
