...
key[KEY_F11]= 0;
key[KEY_ENTER]=0;
uint32_t nkeys = emuIfKeyGetInput(&keymap);

	// Key touch management
	if (nkeys & keymap.scancode[EMU_KEY_UP])  key[KEY_UP]= 1; // UP
	 if (nkeys & keymap.scancode[EMU_KEY_DOWN]) key[KEY_DOWN]= 1; // DOWN
	 if (nkeys & keymap.scancode[EMU_KEY_LEFT]) key[KEY_LEFT]= 1; // LEFT
	 if (nkeys & keymap.scancode[EMU_KEY_RIGHT]) key[KEY_RIGHT]= 1; // RIGHT
	 if (nkeys & keymap.scancode[EMU_KEY_X]) { key[KEY_LCONTROL]=1;  }  // BUTTON A
	 if (nkeys & keymap.scancode[EMU_KEY_O]) { key[KEY_ALT]=1;key[KEY_ENTER]=1;}// key[KEY_5]=1; key[KEY_1]=1;}  // BUTTON B

	 if (nkeys & keymap.scancode[EMU_KEY_TRIANGLE]) { key[KEY_TAB]=1;  }  // BUTTON X
	 if (nkeys & keymap.scancode[EMU_KEY_SQUARE])  {key[KEY_TAB]=1;  }   // BUTTON Y

	if (nkeys & keymap.scancode[EMU_KEY_L])  { key[KEY_LSHIFT]=1;;  }  // BUTTON R
	if (nkeys & keymap.scancode[EMU_KEY_R])  { key[KEY_F11]=1;;  }  // BUTTON L

	 if (nkeys & keymap.scancode[EMU_KEY_START])  { key[KEY_5]=1;  } // START
	 if (nkeys & keymap.scancode[EMU_KEY_SELECT]) { key[KEY_1]=1;key[KEY_TAB]=1 ;} // SELECT
...
//	NativeGE_getKeyInput4Ntv(&keys);
	...
	// Key touch management
	//if (keys.keys&GE_KEY_UP)  OD_B; // UP

	switch (button)
	{
		case 0: return ExKey & OD_B; break;
	//	case 1: return ExKey & (keys.keys&GE_KEY_X); break;
		case 2: return ExKey & OD_Y; break;
		case 3: return ExKey & OD_X; break;
	//	case 4: return ExKey & (keys.keys&GE_KEY_LEFT); break;
	//	case 5: return ExKey & (keys.keys&GE_KEY_RIGHT); break;
		default: break;
	}
	return 0; 
}
