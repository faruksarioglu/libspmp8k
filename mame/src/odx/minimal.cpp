...
/*	ge_key_data_t keys;
	...
	if (keys.keys&GE_KEY_UP)  res |=  OD_UP; // UP
	if (keys.keys&GE_KEY_DOWN) res |=  OD_DOWN; // DOWN
	if (keys.keys&GE_KEY_LEFT) res |=  OD_LEFT; // LEFT
	if (keys.keys&GE_KEY_RIGHT) res |=  OD_RIGHT; // RIGHT

	if (keys.keys&GE_KEY_X) { res |=  OD_A;  }  // BUTTON A
	if (keys.keys&GE_KEY_O) { res |=  OD_B; }  // BUTTON B

	if (keys.keys&GE_KEY_TRIANGLE) { res |=  OD_X;  }  // BUTTON X
	if (keys.keys&GE_KEY_SQUARE)  { res |=  OD_Y;  }   // BUTTON Y

	if (keys.keys&EMU_KEY_R)  { res |=  OD_R;  }  // BUTTON R
	if (keys.keys&GE_KEY_L1)  { res |=  OD_L;  }  // BUTTON L

	if ( keys.keys&GE_KEY_START)  { res |=  OD_START;  } // START
	if ( keys.keys&GE_KEY_SELECT ) { res |=  OD_SELECT; } // SELECT
/*
#ifdef _GCW0_
	// manage joystick
	if (odx_joyanalog) {
...
unsigned long odx_timer_read(void)
{
	//struct timeval tval;
	//gettimeofday(&tval, 0);
	return libgame_utime();//((tval.tv_sec*1000000)+tval.tv_usec);
}
uint8_t *audiobuf;
...
{
//	printf("len:%d",len);
	sp.buf_size=(uint32_t*)len; 
 	sp.buf=buff;//odx_audio_spec.userdata;
 	emuIfSoundPlay(&sp);
...
/*	for(int32_t i = 0; i < len; i += 4) 
	{		
		if(odx_sndlen >= odx_audio_buffer_len) {
		sp.buf=odx_audio_spec.userdata;
		emuIfSoundPlay(&sp);// // just drop samples
		odx_sndlen=0;
		return;
		}
		*(int32_t*)((char*)(odx_audio_spec.userdata + odx_sndlen)) = *(int32_t*)((char*)(buff + i));
...
//	if( odx_audio_spec.userdata) {
	...
	//	*(uint8_t*)((char*)(odx_audio_spec.userdata + odx_sndlen)) = *(uint8_t*)((char*)(buff));
		odx_sndlen += len;
//	}
	
	if(odx_sndlen+len>=odx_audio_buffer_len){
	//sp.buf_size=odx_audio_buffer_len;
	sp.buf=(uint8_t*)odx_audio_spec.userdata;
 	emuIfSoundPlay(&sp);
    odx_sndlen = 0;
		return;
	}
...
void odx_sound_thread_start(void)
{
	...
	odx_audio_buffer_len = 2048; //odx_audio_spec.samples * odx_audio_spec.channels * 2 * 64; 
//	audiobuf = malloc( odx_audio_buffer_len );
//	memset( audiobuf, 0 , odx_audio_buffer_len );
//	odx_audio_spec.userdata=audiobuf;
//	 sp.buf=(uint8_t*)odx_audio_spec.userdata;//.userdata;
...
{
	unsigned int i,l;
	scr=scr+x+y*ODX_SCREEN_WIDTH*2;

	for (i=0;i<strlen(text);i++) {
		
		for (l=0;l<8;l++) {
			scr[l*ODX_SCREEN_WIDTH+0]=(fontdata8x8[((text[i])*8)+l]&0x80)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+0];
			scr[l*ODX_SCREEN_WIDTH+1]=(fontdata8x8[((text[i])*8)+l]&0x40)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+1];
			scr[l*ODX_SCREEN_WIDTH+2]=(fontdata8x8[((text[i])*8)+l]&0x20)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+2];
			scr[l*ODX_SCREEN_WIDTH+3]=(fontdata8x8[((text[i])*8)+l]&0x10)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+3];
			scr[l*ODX_SCREEN_WIDTH+4]=(fontdata8x8[((text[i])*8)+l]&0x08)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+4];
			scr[l*ODX_SCREEN_WIDTH+5]=(fontdata8x8[((text[i])*8)+l]&0x04)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+5];
			scr[l*ODX_SCREEN_WIDTH+6]=(fontdata8x8[((text[i])*8)+l]&0x02)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+6];
			scr[l*ODX_SCREEN_WIDTH+7]=(fontdata8x8[((text[i])*8)+l]&0x01)?odx_palette_rgb[color]:scr[l*ODX_SCREEN_WIDTH+7];
		}
	//	scr+=8;
		scr+=6;
	} 
