...
static uint16_t *DINGOO_GetAudioBuf(_THIS);
...
{
	emu_sound_params_t waveformat;
    waveformat.rate = 44100;
     waveformat.depth = 0;	/* 0 seems to mean 16 bits */
    waveformat.callback = 0;	/* not used for native games, AFAIK */
	waveformat.channels = 2;
	waveformat.buf_size= waveformat.rate/60*waveformat.channels*2;
	waveformat.buf=(uint8_t*)this->hidden->audio_sem;
	emuIfSoundInit(&waveformat);
	emuIfSoundPlay(&waveformat);
	return;
}
uint16_t *DINGOO_GetAudioBuf(_THIS)
{
	return this->hidden->audio_sem;
}
...
int DINGOO_OpenAudio(_THIS, SDL_AudioSpec *spec)
{
	...
	waveformat.rate = spec->freq;
	waveformat.channels = spec->channels;
	waveformat.buf_size= waveformat.rate/60*waveformat.channels*2;
	waveformat.buf=this->hidden;
	//this->hidden->audio_sem=malloc(waveformat.buf_size,sizeof(uint16_t));
	//waveformat.buf=this->hidden;
	
	/* Update the fragment size as size in bytes */
	SDL_CalculateAudioSpec(spec);
  int res=emuIfSoundInit(&waveformat);
