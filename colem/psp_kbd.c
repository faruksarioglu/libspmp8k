...
if (keys.keys & GE_KEY_DOWN) {
      JoyState[joy]&=0xFBFF; break;
	    }
else if (keys.keys & GE_KEY_UP) {
	JoyState[joy]&=0xFEFF; break;
		   }
else if (keys.keys&GE_KEY_RIGHT)
       { 
	   JoyState[joy]&=0xFDFF; break;
	   }
else if (keys.keys&GE_KEY_LEFT){
	JoyState[joy]&=0xF7FF; break;
  	}
else if (keys.keys&GE_KEY_O){
	JoyState[joy] &=0xBFFF; break;
		}
else if (keys.keys&GE_KEY_X){
  	JoyState[joy] &=0xFFBF; break;
	  	}
else if (keys.keys){

JoyState[joy]=0xFFFF;break;
}
			}
  if ((cv_idx >=         0) && 
      (cv_idx <= CV_USER2 )) {
...
psp_kbd_enter_danzeff()
{
  ...
  if ((c.TimeStamp - loc_last_event_time) > KBD_MIN_DANZEFF_TIME) {
    ...
    gp2xCtrlPeekBufferPositive(&c, 1);
    danzeff_key = 1;//danzeff_readInput(c);
  }
...
{
  int wake = 0;
  int reverse_analog = ! CV.psp_joystick_cursor;
    ...
    if (psp_kbd_mapping[psp_b] >= 0) {
      ...
      if (button_pressed) {
        ...
        if (kbd_rtrigger_mapping_active) {
          ...
          cv_key_event(psp_kbd_presses[psp_b], button_pressed);
        cv_key_event(CV_1, button_pressed);
		}
      } else {
...
kbd_scan_keyboard(void)
{
  ...
  if (CV.cv_auto_fire) {
    ...
    if ((delta_stamp < 0) || 
      ...
      cv_key_event(CV_1, CV.cv_auto_fire_pressed);
