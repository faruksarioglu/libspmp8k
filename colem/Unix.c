...
int col = psp_sdl_rgb(0xa0, 0xa0, 0xa0);
 ...
 // if(!InitLibUnix()) { if(Verbose) puts("FAILED");return(0); }

  XBuf = (byte *)malloc(CV_WIDTH * CV_HEIGHT * sizeof(byte));
 psp_sdl_print(50, 60, "InitMachine ok", col);
  psp_sdl_flip();
  //sleep(20);
//SDL_Delay(1000);
  return(1);
...
cv_synchronize(void)
{
  ...
  if (nextclock) {
    u32 curclock;
    do {
     curclock = libgame_utime();//SDL_GetTicks();
...
RefreshScreen(void) 
{
 ...
 //   if (psp_kbd_is_danzeff_mode()) {
   ...
   // }
    ...
    if (CV.cv_view_fps) {
      char buffer[32];
      sprintf(buffer, "%3d", (int)CV.cv_current_fps);
      psp_sdl_fill_print(0, 0, buffer, 0xffffff, 0 );
    }

    if (CV.psp_display_lr) {
