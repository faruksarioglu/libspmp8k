...
#include "psp_danzeff.h"
  ...
  SDL_Surface *splash_surface=NULL;
  SDL_Surface *thumb_surface=NULL;
...
{
  back2_surface = IMG_Load("/fat20a2/hda2/GAME/colem/background.png");
  thumb_surface = IMG_Load("/fat20a2/hda2/GAME/colem/thumb.png");
}
...
{
 // if (! splash_surface) {
    splash_surface = IMG_Load("/fat20a2/hda2/GAME/colem/splash.png");
 // }
  SDL_Rect offset;
    offset.x = 0;
    offset.y = 0;
    SDL_FillRect( back_surface, &back_surface->clip_rect, SDL_MapRGB( back_surface->format, 0xCC, 0xCC, 0xFF ) );
	SDL_BlitSurface(splash_surface, &splash_surface->clip_rect, back_surface,&offset);
}
...
sleep(200);
  //psp_sdl_blit_splash();
 // psp_sdl_print(50, 50, "Splash", col);
 // psp_sdl_flip();
//sleep(200);
...
SDL_Flip(back_surface);
 ...
 //danzeff_load();
  ...
  SDL_Delay(10);        // Give sound some time to init
  //SDL_PauseAudio(1);
